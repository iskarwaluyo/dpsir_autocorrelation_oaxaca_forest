...
observe({
  ...
  if (input$leyenda) {
    proxy %>% 
      addLegend(m0, "topleft", group = "MADERABLE", pal = pal_vpm, values = ac_mapa_maderable$VPM_2016, opacity = 1.0, title = "Producción madreable") %>%
      addLegend(m0, "topleft", group = "NO MADERABLE", pal = pal_vpnm, values = ac_mapa_maderable$VPNM_2016, opacity = 1.0, title = "Producción no-madreable")
