function(input, output, session) {
  ...
  observe({
    proxy <- leafletProxy("mapa_pressure", data = ac_mapa_agricola)
    proxy %>% clearControls()
    if (input$leyenda) {
      proxy %>% 
  ...
  observe({
    proxy <- leafletProxy("mapa_state", data = ac_mapa_ganadera)
    proxy %>% clearControls()
    if (input$leyenda) {
      proxy %>% 
  ...
  observe({
    proxy <- leafletProxy("mapa_impact", data = ac_mapa_maderable)
    proxy %>% clearControls()
    if (input$leyenda) {
      proxy %>% 
        addLegend(m3, "topleft", group = "APROVECHAMIENTO MADERABLE", pal = pal_vpm, values = ac_mapa_maderable$APM_2016, opacity = 1.0, title = "Timber forest production <br/> (cubic meters)")
        addLegend(m3, "topleft", group = "APROVECHAMIENTO NO MADERABLE", pal = pal_vpnm, values = ac_mapa_maderable$APNM_2016, opacity = 1.0, title = "Non timber forest production <br/> (tons)")
    }
  })

  observe({
    proxy <- leafletProxy("mapa_response", data = ac_mapa_psa)
    proxy %>% clearControls()
    if (input$leyenda) {
      proxy %>% 
